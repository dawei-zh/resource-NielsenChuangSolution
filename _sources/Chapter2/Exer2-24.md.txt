...
$$
B = \frac{A + A^{\dagger}}{2}, C = \frac{A - A^{\dagger}}{2i} = \frac{iA^{\dagger} - iA}{2} 
$$(eqn:2.24.1)
and with above construction, we can recover $A$ by

$$
B + iC = \frac{A + A^{\dagger}}{2} +i\frac{A - A^{\dagger}}{2i} = \frac{A + A^{\dagger}}{2} +\frac{A - A^{\dagger}}{2} = A 
$$(eqn:2.24.2)

It is easy to verify that $B$ and $C$ are Hermitian, since

$$
B^{\dagger} = \frac{A^{\dagger}+A}{2} = B, C^{\dagger} = \frac{-iA +i A^{\dagger}}{2} = C
$$(eqn:2.24.3)

where we use

$$
\left(\sum_{i} a_i A_i\right)^{\dagger} = \sum_{i} a^{*}_i A^{\dagger}_i
$$(eqn:2.24.4)
...
For positive operator $A$, we should have $\langle v|A|v\rangle  >0$ is a real number. So from eq. {eq}`eqn:2.24.6` we have

$$
\langle v|A|v\rangle =  \sum_{i}|v_{bi}|^2\lambda_i  + i\sum_{x}|v_{cx}|^2\mu_x >0 \iff \lambda_i \geq 0, \mu_x = 0 
$$(eqn:2.24.7)

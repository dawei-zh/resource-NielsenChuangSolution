...
$$
|v_{k+1}\rangle = \frac{|w_{k+1}\rangle - \sum_{i=1}^{k}\langle v_{i}|w_{k+1}\rangle |v_{i}\rangle}{|||w_{k+1}\rangle - \sum_{i=1}^{k}\langle v_{i}|w_{k+1}\rangle |v_{i}\rangle||}
$$(eqn:2.8.1)
...
* For $|v_1\rangle$, we have $|v_1\rangle = |w_1\rangle / |||w_1\rangle||$, then 
  ...
  $$
  \langle v_1|v_1\rangle = \frac{\langle w_1|w_1\rangle} {|||w_1\rangle||^2} = 1
  $$(eqn:2.8.2)

* For $n=1$, we have

  $$
  |v_{2}\rangle = \frac{|w_{2}\rangle - \langle v_{1}|w_{2}\rangle |v_{1}\rangle}{|||w_{2}\rangle - \langle v_{1}|w_{2}\rangle |v_{1}\rangle||}
  $$(eqn:2.8.3)

  Suppose $|a\rangle = |w_{2}\rangle - \langle v_{1}|w_{2}\rangle |v_{1}\rangle$ then 

  $$
  \langle v_2|v_2\rangle = \frac{\langle a|a\rangle}{|||a\rangle||^2} = 1
  $$(eqn:2.8.4)

  Meanwhile, we have

  $$
  \langle v_1|v_{2}\rangle = \frac{\langle v_1|w_{2}\rangle - \langle v_{1}|w_{2}\rangle \langle v_1|v_{1}\rangle}{|||w_{2}\rangle - \langle v_{1}|w_{2}\rangle |v_{1}\rangle||} = 0
  $$(eqn:2.8.5)
...
* Suppose $n=k$ if we have $\langle v_{j}|v_{k+1}\rangle = \delta_{(k+1)j}$, then when $n=k+1$, for $j\neq k+2$, 
  ...
  $$
  \langle v_{k+2}|v_{k+2}\rangle = \frac{\langle a|a\rangle}{|||a\rangle||^2} = 1 
  $$(eqn:2.8.7)

  From eq. {eq}`eqn:2.8.6` and eq. {eq}`eqn:2.8.7`, we conclude that for $n=k+1$, $\langle v_{j}|v_{k+2}\rangle = \delta_{(k+2)j}$ for $j\leq k+2$. 
